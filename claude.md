...
| 2026-04 | CV Evaluation → Set Criteria → "From Job Description" always said "This job has no description" even after the JD was saved | `/job-openings` list workflow (`phase1-job-opening`, "List - Build Query" node) SELECTed an explicit column list that omitted `job_description`; frontend's `evalSelectedJob` was populated from that list and never re-fetched the full row, so `job_description` was always `undefined` | Added `job_description` to the SELECT column list, re-imported the workflow, published + restarted n8n so the new active version was picked up. Frontend unchanged |
